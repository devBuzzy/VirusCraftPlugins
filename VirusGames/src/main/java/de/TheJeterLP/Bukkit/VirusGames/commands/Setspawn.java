...
public class Setspawn extends SubCommand {
    ...
    public CommandResult executePlayer(Player p, CommandArgs args) throws SQLException {
        ...
        String spawn;
        if (args.getString(1).equalsIgnoreCase("player")) {
            spawn = "player";
        } else if (args.getString(1).equalsIgnoreCase("virus")) {
            spawn = "virus";
        ...
        PreparedStatement st = VirusGames.getDB().getPreparedStatement("UPDATE `maps` SET `" + spawn + "` = ? WHERE `id` = ?;");
        st.setString(1, Utils.serialLocation(p.getLocation()));
        st.setInt(2, id);
        st.executeUpdate();
        st.close();
        VirusGames.getMessageManager().message(p, MessageManager.PrefixType.GOOD, "The " + spawn + " spawn was set to your position.");
        return CommandResult.SUCCESS;
