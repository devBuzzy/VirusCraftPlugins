 ...
 * @author TheJeterLP
 */
public class Command_God extends BaseCommand {
    
    public Command_God() {
        super("god");
        helpPages.add(new CommandHelp("/god <player>", "Toggles yours or <player>'s godmodea"));
    }
    ...
    public CommandResult onPlayerCommand(Player p, Command cmd, CommandArgs args) throws Exception {
        if (args.isEmpty()) {
            ...
            vcp.setGod(!vcp.god());
            String s = vcp.god() ? "enabled" : "disabled";
            return Utils.sendMessage(MessageType.INFO, p, "God mode has been " + s);
        } else {
            ...
            vctarget.setGod(!vctarget.god());
            String s = vctarget.god() ? "enabled" : "disabled";
            Utils.sendMessage(MessageType.INFO, p, vctarget.getDisplayName() + "'s god mode is now " + s);
            return Utils.sendMessage(MessageType.INFO, target, "God mode has been " + s);
        }
    }
