...
public class Command_Unmute extends BaseCommand {
    ...
    public Command_Unmute() {
        super("unmute");
        helpPages.add(new CommandHelp("/unmute <player>", "Unmutes <player>"));
    }
    ...
    public CommandResult onServerCommand(CommandSender sender, Command cmd, CommandArgs args) throws Exception {
        ...
        if (!vctarget.isMuted()) return Utils.sendMessage(MessageType.ERROR, sender, vctarget.getDisplayName() + " is not muted.");
        vctarget.setMuted(false);
        vctarget.sendMessage(MessageType.INFO, "You have been unmuted.");
        return Utils.sendMessage(MessageType.INFO, sender, vctarget.getDisplayName() + " has been unmuted.");
    }
