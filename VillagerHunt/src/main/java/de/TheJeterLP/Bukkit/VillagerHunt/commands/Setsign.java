...
public class Setsign extends SubCommand {
    ...
    public CommandResult executePlayer(Player p, CommandArgs args) throws SQLException {
        ...
        if (!(Utils.getBlockLooking(p, 10).getState() instanceof Sign)) {
            VillagerHunt.getMessageManager().message(p, MessageManager.PrefixType.BAD, "That Block is not a sign!");
            return CommandResult.SUCCESS;
        }
        Location loc = Utils.getLocationLooking(p, 10);

        PreparedStatement st = VillagerHunt.getDB().getPreparedStatement("UPDATE `maps` SET `sign` = ? WHERE `id` = ?;");
        st.setString(1, Utils.serialLocation(loc));
        st.setInt(2, id);
        st.executeUpdate();
        st.close();
        VillagerHunt.getMessageManager().message(p, MessageManager.PrefixType.GOOD, "The sign was set to your position.");
        return CommandResult.SUCCESS;
    }
