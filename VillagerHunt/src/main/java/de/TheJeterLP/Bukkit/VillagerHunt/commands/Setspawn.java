...
public class Setspawn extends SubCommand {
    ...
    public CommandResult executePlayer(Player p, CommandArgs args) throws SQLException {
       ...
       String spawn;
        ...
        PreparedStatement st = VillagerHunt.getDB().getPreparedStatement("UPDATE `maps` SET `" + spawn + "` = ? WHERE `id` = ?;");
        st.setString(1, Utils.serialLocation(p.getLocation()));
        st.setInt(2, id);
        st.executeUpdate();
        st.close();
        VillagerHunt.getMessageManager().message(p, MessageManager.PrefixType.GOOD, "The " + spawn + " spawn was set to your position.");
        return CommandResult.SUCCESS;
